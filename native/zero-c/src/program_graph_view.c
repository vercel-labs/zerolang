...
static bool view_module_is_stdlib(const ZProgramGraphNode *node) {
  return node &&
         node->kind == Z_PROGRAM_GRAPH_NODE_MODULE &&
         (view_text_starts_with(node->name, "std.") || view_text_starts_with(node->path, "std/"));
}
