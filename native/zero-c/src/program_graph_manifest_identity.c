...
bool z_program_graph_manifest_main_path(const char *input_path, char **out, ZDiag *diag) {
  if (out) *out = NULL;
  char *manifest_path = z_manifest_path_for_input(input_path);
  if (!manifest_path) return true;
  ...
  if (out && parsed.main_path && parsed.main_path[0]) {
    *out = z_strdup(parsed.main_path);
  }
