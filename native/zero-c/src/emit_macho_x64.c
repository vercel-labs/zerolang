...
static bool machx64_emit_proc_child_spawn_value(ZBuf *text, const IrFunction *fun, const IrValue *value, MachOEmitContext *ctx, ZDiag *diag) {
  ...
  if (!machx64_emit_runtime_call(text, ctx, value->index ? MACHO_RUNTIME_PROC_SPAWN_CHILD_IN_ENV : (value->right ? MACHO_RUNTIME_PROC_SPAWN_CHILD_IN : MACHO_RUNTIME_PROC_SPAWN_CHILD), abi_slots, abi_slots, temp_base, value, diag)) return false;
  z_x64_emit_add_rsp(text, total_stack);
  return true;
}
