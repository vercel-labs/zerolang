...
assert.equal(fixPlanBody.fixes[0].appliesEdits, false);
assert.equal(fixPlanBody.fixes[0].hasPreview, true);
assert.equal(Array.isArray(fixPlanBody.fixes[0].edits), true);
assert.equal(fixPlanBody.fixes[0].edits.length, 1);
const fixPlanEdit = fixPlanBody.fixes[0].edits[0];
assert.equal(fixPlanEdit.path, "conformance/native/fail/mem-copy-immutable-dst.0");
assert.equal(fixPlanEdit.range.start.line, 2);
assert.equal(fixPlanEdit.range.start.column, 5);
assert.equal(fixPlanEdit.range.end.line, 2);
assert.equal(fixPlanEdit.range.end.column, 8);
assert.equal(fixPlanEdit.range.columnUnit, "utf8-byte");
assert.equal(fixPlanEdit.oldText, "let");
assert.equal(fixPlanEdit.newText, "let mut");
assert.equal(fixPlanEdit.precondition.kind, "exact-text");
assert.equal(fixPlanEdit.precondition.text, "let");
...
assert.equal(genericFixPlanBody.fixes[0].hasPreview, false);
assert.equal(genericFixPlanBody.fixes[0].edits, undefined);
