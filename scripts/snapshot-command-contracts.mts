...
assert.match(graphHelp, /zero graph patch \[--json\] --out <program-graph> <input> <patch-file>/);
...
assert.match(rootHelp, /zero graph patch \[--json\] --out <program-graph> <graph-artifact-or-package> <patch-file>/);
...
assert.equal(graphBareOutJson.body.diagnostics[0].expected, "zero graph dump|import|validate|patch|roundtrip --out <program-graph> <input>");
